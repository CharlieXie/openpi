...
└── configs/
    ├── waypoint_joint_libero.yaml      # LIBERO 联合训练配置 — 主配置
    ├── eval_waypoint_joint_libero.yaml # LIBERO 评测配置 (联合模型)
...
## 七、Action Expert 架构参考 (`ae_model.py` — 独立模型，仅供参考)

> **注意**: 联合模型 `joint_model.py` 已包含 AE 的完整逻辑（`ae_forward()` + `sample_actions()`）。本章仅作为架构参考，描述 AE 相对于 Pi0.5 base 的设计变化。
...
## 八、VLM 架构参考 (`vlm_model.py` — 独立模型，仅供参考)

> **注意**: 联合模型 `joint_model.py` 已包含 VLM 的完整逻辑（`vlm_forward()` + `generate_waypoints()`）。本章仅作为架构参考。
...
# 否则 257K 词表中 pretrained 的语言 token 先验会压过 292 个专用 waypoint token
...
评测配置 (`configs/eval_waypoint_joint_libero.yaml`) 中需设置联合 checkpoint 路径：
```yaml
...
独立 AE/VLM 训练脚本 `scripts/train_waypoint.py` 仍可用，适合调试或单独训练某一组件：

```bash
# AE: torchrun scripts/train_waypoint.py --mode ae --config configs/waypoint_ae_libero.yaml
# VLM: PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True .venv/bin/torchrun ... --mode vlm --config configs/waypoint_vlm_libero.yaml
# 独立评测: --config configs/eval_waypoint_libero.yaml (需分别指定 vlm_checkpoint 和 ae_checkpoint)
```
...
### `waypoint_joint_libero.yaml` (主配置)
...
- `waypoint_ae_libero.yaml` — AE 独立训练，使用 `scripts/train_waypoint.py --mode ae`
- `waypoint_vlm_libero.yaml` — VLM 独立训练，使用 `scripts/train_waypoint.py --mode vlm`
...
独立训练使用 Project `waypoint_vla`，记录 `train/loss` (MSE for AE, CE for VLM)、`train/lr`、`train/grad_norm`、`train/steps_per_sec`。
...
- 正确做法：手动循环 state_dict，遇到 shape 不匹配的 key 跳过并打印 warning（见 `PI0WaypointJoint.load_pretrained_weights()`）
...
- 联合模型中 VLM 路径使用 HuggingFace 的 `PaliGemmaForConditionalGeneration`
- 必须通过 HuggingFace API 激活 checkpointing：`self.paligemma.gradient_checkpointing_enable()`
...
- **修复 (vlm_model.py / joint_model.py)**: proprio 位置仅允许 token 256768–257023，duration 位置仅允许 token 256734–256767，其余设为 `-inf`
...
每个 rank 独立运行 DataLoader，`batch_size` 是**单 GPU** 的 batch 大小。联合训练中 VLM 和 AE 的 dataloader 分别有独立的 batch size。

### Action Expert (联合训练中的 `ae_batch_size`)
...
配合 LR 线性缩放规则: `peak_lr = 5e-5 × (new_batch / 64)`

### VLM (联合训练中的 `vlm_batch_size`)

VLM 全量 finetune PaliGemma 2B。
...
| batch_size (per GPU) | 实测 GPU 内存 | 有效总 batch | 备注 |
|----------------------|--------------|-------------|------|
| 12 | ~91-93 GB | 24 | 独立训练 DDP 极限 |
| 192 | 待测 | 384 | 当前联合训练配置 |

---

## 十四、联合训练 (Joint VLM + AE)
...
| `ae_forward(obs, start_proprio, end_proprio, actions, duration, ...)` | MSE flow-matching loss，含梯度策略支持 | 移植自 `ae_model.py:166-242` |
...
共享 backbone 时，VLM 和 AE 的梯度可能冲突：

**冲突来源**: VLM 使用 Cross-Entropy loss (离散分类)，AE 使用 MSE loss (连续回归)。两种 loss 对 backbone 同一组权重产生方向不同的梯度更新：
- CE 梯度希望 backbone 的表征适合离散 token 分类 (锐化 logits 分布)
- MSE 梯度希望 backbone 的表征适合连续动作回归 (平滑 flow-matching 输出)

**冲突路径**: AE 的 MSE 梯度通过 cross-attention 的 K/V 投影流入 backbone。具体来说，Expert (Gemma 300M) 的 query 会 attend to backbone 的 key/value，MSE loss 的梯度沿此路径回传到 backbone 的 `k_proj` 和 `v_proj` 权重。

**与 Pi0.5 KI 论文的关系**: Pi0.5 §5.2 的 Knowledge Insulation (KI) 正是为了解决此问题——通过 `sg(K_b)` / `sg(V_b)` 完全阻断 MSE 梯度进入 backbone。但本项目与 Pi0.5 有一个关键差异：Pi0.5 的 VLM 使用 FAST tokens (1024 bins × 多 action chunk)，信息密度高；而 Waypoint VLA 的 VLM 只预测 7 个稀疏 waypoint (每个 10 tokens = 70 tokens)，信息密度较低。完全 KI 可能导致 backbone 从 VLM CE loss 获得的 action 表征信号不足。因此本项目提供了 `scale_gradient` 作为 soft KI 选项。

### 梯度策略

通过构造函数参数 `gradient_strategy` 控制，支持四种模式：
...
在 `gemma_pytorch.py` 的 `compute_layer_complete` (line 159) 中，每层 attention 计算前对 backbone 的 K 和 V 施加 detach：
...
#### `"scale_gradient"` — 软 Knowledge Insulation

在 `ae_forward()` 中，通过 `knowledge_insulation=gradient_scale` (如 0.3) 传给 backbone。
在 `gemma_pytorch.py` 的 `compute_layer_complete` (line 159) 中实现：
...
**数学含义**: 前向传播值不变 (`s*K + (1-s)*K.detach() = K`)，但反向传播时 AE MSE 梯度通过 K_b/V_b 传入 backbone 的幅度缩放为原来的 `scale` 倍。

**效果**: backbone 收到 `∇CE + scale * ∇MSE_via_KV`。VLM CE loss 是主导训练信号，AE MSE loss 提供辅助调节，避免完全隔离导致 backbone 表征不足。

**适用场景**: VLM 的离散 waypoint tokens 信息密度较低（比 Pi0.5 的 FAST tokens 稀疏），完全 stop_gradient 可能让 backbone 学不到足够的 action 表征。scale_gradient 允许 AE 梯度以受控的比例慢慢引导 backbone 适应。

**推荐配置**: `gradient_scale: 0.1~0.3`
...
**显存优势**：联合模型只加载一份 backbone，推理时约 ~14 GB vs 独立模型 ~19 GB。
